...
from app.timeline.schemas.reminder import ReminderCreate, ReminderUpdate, DoseScheduleCreate
...
@router.post("/{pet_id}/reminders/generate-dose-schedule", status_code=201)
async def generate_dose_schedule(pet_id: str, body: DoseScheduleCreate,
                                  user=Depends(get_current_user)):
    """
    Generate one reminder per time-slot per day for a full medication course.
    E.g. frequency=["morning","night"], duration_days=7 → 14 reminders created.
    Dose labels per slot: {"morning": "1 tablet", "night": "2 tablets"}.
    """
    _guard(pet_id, user)
    try:
        return RE.generate_dose_schedule(pet_id, body.model_dump(mode="json"))
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
@router.put("/{pet_id}/reminders/{rid}/mark-missed")
async def mark_missed_r(pet_id: str, rid: str, user=Depends(get_current_user)):
    """Explicitly mark a reminder as missed (e.g. from a timeline view action)."""
    _guard(pet_id, user)
    r = RE.mark_missed(pet_id, rid)
    if not r:
        raise HTTPException(404, "Reminder not found")
    return r
