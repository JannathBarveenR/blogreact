...
- **Status/Crash Warning**: These endpoints query the `daily_task_logs` and `pet_streaks` tables. Since these tables do not exist in the live Supabase database schema, invoking any checklist router endpoints directly will trigger a database connection crash/missing table error.

---

## 4. Exhaustive Database Schema (Supabase)

The live Supabase instance (`https://olmpcmvgvdsgvnwiriut.supabase.co`) contains the following tables and schemas as verified by schema inspection metadata:

### 4.1. Core Tables

**`user_profiles`** (Extended user metadata linked to auth)
- `id`: UUID (Primary Key, FK to `auth.users.id` with `ON DELETE CASCADE`).
- `full_name`: Text (User display name).
- `phone`: Text (Registered mobile number).
- `email`: Text (Registered email address).
- `phone_verified`: Boolean (OTP check flag).
- `auth_provider`: Text (e.g. `email` or `google`).
- `city`, `state`, `pincode`: Text (User address/location details).
- `avatar_url`: Text (Public photo URL from `avatars` bucket).
- `created_at`, `updated_at`: Timestamp.

**`pet_profiles`** (Core pet profile records)
- `id`: UUID (Primary Key, autogenerated).
- `user_id`: UUID (Foreign Key to `user_profiles.id` with `ON DELETE CASCADE`).
- `petolife_id`: Text (Unique custom generated identifier).
- `pet_type`: Text (Dog, Cat, Rabbit, etc.).
- `pet_name`: Text.
- `breed`: Text.
- `gender`: Text.
- `birth_date`: Date.
...
- `pet_photo_url`: Text (Public photo URL from `pet-photos` bucket).
- `household_id`: UUID (Linked family household).
- `pet_health_id`: Text.
- `city`, `state`, `pincode`: Text (Registered city, state, postal code).
- `owner_name`: Text.
- `owner_phone`: Text.
- `created_at`, `updated_at`: Timestamp.
...
- `id`: UUID (Primary Key).
- `pet_profile_id`: UUID (Foreign Key to `pet_profiles.id` with `ON DELETE CASCADE`).
- `id_name`: Text (e.g., "Microchip", "KCI Registration").
- `id_number`: Text (Registration sequence).
...
- `user_id`: UUID (Foreign Key to `user_profiles.id` with `ON DELETE CASCADE`).
- `title`: Text.
- `category`: Text (e.g. Prescription, Vaccination, Lab Reports).
- `file_url`: Text.
- `file_name`: Text.
- `file_type`: Text (e.g. application/pdf, image/png).
- `file_size`: Integer (Bytes).
- `storage_path`: Text (Relative location in the storage bucket).
- `is_favorite`: Boolean (Toggles favorite flag).
- `created_at`: Timestamp.

**`pet_health_ids`** (Sequential Health ID Generator Log)
- `id`: UUID (Primary Key).
- `health_id`: Text (Unique, custom generated string).
- `pet_profile_id`: UUID (Foreign Key to `pet_profiles.id` with `ON DELETE CASCADE`).
- `city_code`: Text (3-letter city code).
- `pet_type_code`: Text (3-letter pet type).
- `sequence_number`: Integer (Counter).
- `pet_name`: Text.
- `owner_phone`: Text.
- `created_at`: Timestamp.

**`early_access_registrations`** (Landing Page Interests Form leads)
- `id`: UUID (Primary Key).
- `type`: Text (`pet_parent` or `veterinarian`).
- `name`, `clinic_name`, `mobile`, `email`, `city`, `pet_type`: Text.
- `early_access`: Boolean.
- `created_at`: Timestamp.

**`task_activity_log`** (Household audit trails)
- `id`: UUID (Primary Key).
- `household_id`: UUID.
- `pet_id`: UUID (Foreign Key to `pet_profiles.id` with `ON DELETE CASCADE`).
- `task_id`, `task_title`, `member_name`, `action`: Text.
- `member_id`: UUID.
- `timestamp`: Timestamp.

**`pet_parents` & `vets`** (Marketing / Campaign lead captures)
- `id`: UUID (Primary Key).
- `name`/`doctor_name`, `mobile`, `email`, `city`: Text.
- `clinic_name`, `pet_type`, `pet_name`: Text (role-specific parameters).
- `has_pet`, `early_access`: Boolean.
- `created_at`: Timestamp.

**`event_registrations_coimbatore`** (Coimbatore launch campaign entries)
- `id`: UUID (Primary Key).
- `pet_name`, `owner_name`, `owner_phone`, `pet_health_id`: Text.
- `linked`: Boolean.
- `linked_user_id`: UUID.
- `created_at`: Timestamp.
...
3. **`medical-docs`**: Stores PDF/JPG/PNG files uploaded from the Medical Records tab. *(Note: Backend router references `medical-docs` bucket name).*
